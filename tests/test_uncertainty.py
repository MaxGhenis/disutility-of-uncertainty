...
def test_welfare_loss_calculation():
    ...
    loss = analysis.calculate_welfare_loss(wage, true_tax, perceived_tax, transfers)

    # Loss should be positive when there's misperception
    assert loss > 0

    # Loss should be zero when perception is correct
    loss_zero = analysis.calculate_welfare_loss(wage, true_tax, true_tax, transfers)
...
def test_deadweight_loss():
    ...
    dwl = analysis.deadweight_loss_from_misperception(wage, tax_rate, misperception_sd)

    # DWL should be positive with misperception
    assert dwl > 0

    # DWL should be zero with no misperception
    dwl_zero = analysis.deadweight_loss_from_misperception(wage, tax_rate, 0.0)
    assert abs(dwl_zero) < 1e-10

    # DWL should increase with misperception
    dwl_high = analysis.deadweight_loss_from_misperception(wage, tax_rate, 0.2)
    assert dwl_high > dwl
