...
class CobbDouglasUtility(UtilityFunction):
    ...
    def marginal_utility_leisure(self, leisure: float, consumption: float) -> float:
        ...
        if leisure == 0:
            return np.inf
        return self.leisure_exponent * (leisure ** (self.leisure_exponent - 1)) * \
    ...
    def marginal_utility_consumption(self, leisure: float, consumption: float) -> float:
        ...
        if consumption == 0:
            return np.inf
        return self.consumption_exponent * (leisure ** self.leisure_exponent) * \
    ...
    def marginal_rate_substitution(self, leisure: float, consumption: float) -> float:
        ...
        return (self.leisure_exponent / self.consumption_exponent) * (consumption / leisure)
...
class OptimalChoice:
    ...
    def optimal_leisure(
        ...
        if not isinstance(self.utility_function, CobbDouglasUtility):
            raise NotImplementedError("Only Cobb-Douglas utility is currently supported")
